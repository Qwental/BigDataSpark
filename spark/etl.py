...
from pyspark.sql.functions import (
    col, to_date, row_number, month, year,
...
dim_product = (
    ...
    .select(
        ...
        "product_rating", "product_reviews", "product_brand",
...
dim_supplier = (
    ...
    .select(
        ...
        "supplier_phone", "supplier_city", "supplier_country"
    )
...
all_views = [
    "v_prod_top10", "v_prod_rev_by_category", "v_prod_avg_stats",
    "v_cust_top10", "v_cust_by_country", "v_cust_avg_check",
    "v_time_monthly_trends", "v_time_yearly_compare", "v_time_avg_order_by_month",
    "v_store_top5", "v_store_by_geo", "v_store_avg_check",
    "v_supp_top5", "v_supp_avg_price", "v_supp_by_country",
    "v_qual_top_rated", "v_qual_low_rated", "v_qual_most_reviewed",
]
for v in all_views:
...
ch_execute("""
CREATE VIEW v_prod_rev_by_category AS
SELECT
    product_category,
    SUM(sale_total_price) AS total_revenue,
    COUNT(*)              AS num_sales
...
ch_execute("""
CREATE VIEW v_prod_avg_stats AS
SELECT
    product_name,
    product_brand,
...
FROM base_customers
GROUP BY customer_id, customer_first_name, customer_last_name, customer_email
...
ch_execute("""
CREATE VIEW v_time_yearly_compare AS
SELECT
    year,
...
# 6. Витрина качества
ch_execute("""
CREATE VIEW v_qual_top_rated AS
SELECT
    product_name,
    product_category,
    product_rating,
...
ch_execute("""
CREATE VIEW v_qual_low_rated AS
SELECT
    product_name,
    product_category,
    product_rating,
